...
def monophyly_prune(tree):
    ...
    if len(invalid_names) > 0:
        raise ValueError(
            f"Tip name {invalid_names[0]} does not conform to taxon@id naming requirement."
        )
...
def split_tree(tree, nodes):
    ...
    for leafset in leafset_of_new_trees:
        if not leafset:
            continue
        induced_tree = tree.extract_tree(node_filter_fn=lambda nd: nd in leafset)
...
def paralogy_prune(tree, pruned_trees):
    ...
    for node in tree.postorder_node_iter():
        ...
        counts[node] = max(count_orthologs(child_leaves), count_orthologs(other_leaves))
    ...
    if (splits[0] == tree.seed_node) and (n_species > 1):
        ...
        assert tree.seed_node not in splits
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="prune one or more gene trees into maximally inclusive subtrees with no more than one tip per species"
    )
    parser.add_argument(
        "input",
        help="input tree file, containing one or more phylogenies in newick format",
    )
    parser.add_argument(
        "output", help="output tree file, containing all the pruned subtrees"
    )
    parser.add_argument(
        "-d",
        "--disablemonophylyprune",
        help="disable monophyly_prune prior to paralogy_prune",
        action="store_true",
    )
    parser.add_argument(
        "-m",
